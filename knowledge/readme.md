...
###### 一点补充
```JS
- 一般情况下，原始类型的数据值保存在“栈”中，引用类型的值存放在“堆”中；
  - 特殊情况，例如字符串太大、bigInt 这些会存在堆中（看引擎实现）；

- 闭包，当 JS引擎编译函数内部代码时，如果遇到内部函数引用外部函数的变量，会判断这是一个闭包；
  - 然后在堆中创建一个对象存放这些引用的变量，此时，内部函数会引用这个存放变量的对象；
